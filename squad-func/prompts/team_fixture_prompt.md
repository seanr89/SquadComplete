...
2. **Data Extraction:** Identify the starting XI for both teams only. strictly limit positions to a max of 11 for each team and only starters not subs, Goalkeeper, Defender, Midfielder, Forward abberivated to G, D, M, F
3. **Rating Retrieval:** Extract the individual player performance ratings (typically on a 1-10 scale). If ratings are not available for this specific league/match, get the players average season rating for that season and use that rating.
